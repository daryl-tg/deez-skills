...
| `dev-notes/agents/om-chat-feature` | **Dissolved.** 214 lines became `om-mode/playbooks/om-chat-feature.md` plus `references/om-chat-boundaries.md` |
| `dev-notes/agents/om-mobile-feature` | **Dissolved.** 343 lines became `om-mode/playbooks/om-mobile-feature.md` plus `references/openfloor-lane.md` |
| `dev-notes/agents/om-*-completion` | **Dissolved** into `om-mode/playbooks/om-chat-completion.md` and `om-mobile-completion.md` |
| `dev-notes/agents/om-*-loop` | **Dissolved** into `om-mode/playbooks/agentic-loop.md` |
| `dev-notes/agents/om-agentic-loop`, `-stub` | **Retired.** A loop is a playbook, not a second router |
| `dev-notes/agents/om-agent-polish` | **Retired.** Evidence contract moved to the terminal gate and `control-<app> evidence publish`; the audit checklist belongs with `om-chat-design-system` |
...
3. `bin/link --apply` on one machine. Verify both runtimes still see every skill
   by name, and that `$om-mode` resolves.
...
6. Update `~/.claude/CLAUDE.md`, which still names `dev-notes/agents` as
   canonical, `tools/skills-*.sh` as the tooling, and `$om-agent-polish` as a
   skill to load.
