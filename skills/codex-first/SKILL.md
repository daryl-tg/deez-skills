...
already decided to hand off becomes a Codex run. The agentic-loop playbook calls
...
Inside the agentic-loop playbook, the handoff decision is already made: invoking
...
The agentic-loop playbook's validation-ownership table states, so the two files
cannot disagree:
...
## Goal-contract handoff (dispatch from the agentic-loop playbook)

When the agentic-loop playbook dispatches a goal here, the prompt carries the
...
The agentic-loop playbook's profile at
`~/github/deez-skills/skills/clanker-mode/playbooks/agentic-loop.md`,
...
om families — `$om-chat-feature`, the agentic-loop playbook, `$om-mobile-feature` —
...
Validation and review ownership belong to the agentic-loop playbook's
validation-ownership table, not to this skill — see
`~/github/deez-skills/skills/clanker-mode/playbooks/agentic-loop.md` for what
