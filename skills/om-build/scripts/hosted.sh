...
if [ "$NO_GUI" = 0 ]; then
  ...
  # private scope); this is the one that bites now that the token is good. The
  # GUI then fails to build against its own merged commits with MISSING_EXPORT,
  # or worse, builds green against a stale wire contract.
  if ( cd "$GUI" && bun run rooms-client:status 2>&1 | grep -q "registry copy" ); then
    say "rooms-client:          relinking (bun install replaced the link)"
    ( cd "$GUI" && OM_REPO="$MONO" bun run rooms-client:link ) >/dev/null 2>&1 || die "rooms-client relink"
  fi
...
# Poll, never a fixed sleep. 0.161.1 takes ~20s from shutdown to bound port
# (collab bridge, librarian, TLS channel, library mirror, secrets mailboxes), so
# `sleep 15` reported a perfectly good build as "daemon not answering".
BOOTED=0
printf 'waiting for daemon'
for _ in $(seq 1 45); do
  if curl -s -o /dev/null -m 2 "$HEALTH"; then BOOTED=1; break; fi
  printf '.'; sleep 2
done
printf '\n'
