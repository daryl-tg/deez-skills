...
- `inbox-filter` opens the filter drawer beside search.
- `inbox-alerts` surfaces ringing feeds.
- `inbox-activity` opens notifications and requests.
- `inbox-new` expands the `+` into New Group / New Server / Connections.
...
| `label="Alerts, checking"` / `label="Alerts: 5 feeds are ringing"` | alerts, **text varies with state** |
...
- **Search.** Run
  ...
  *"Press Search to look inside channel and DM message history."* — filtering
  ...
  `label="ger"`. To clear it, use `./control-openfloor app reset` — **never** the
  keyboard `delete` key, and `fill … ""` is rejected.
- **Open activity.** Run
...
- **New-conversation menu.** Run
  ...
  The settled diff should add New Group / New Server / Connections. Opening the
  menu is free; creating anything is not.
