...
class RoutingAgreementTest(unittest.TestCase):
    """registry.toml's default_lane and what the mode skill tells an agent
    must agree. They drifted once: the config said codex, the router's role
    table showed both columns, and every run stayed on Claude."""

    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory()
        self.addCleanup(self.tmp.cleanup)
        self.repo = Path(self.tmp.name) / "repo"
        (self.repo / "skills" / "the-mode").mkdir(parents=True)

    def build(self, lane, body):
        (self.repo / "skills" / "the-mode" / "SKILL.md").write_text(
            f"---\nname: the-mode\ndescription: d\n"
            f"disable-model-invocation: true\n---\n\n{body}\n")
...
""")
    ...
    def test_agreement_passes(self):
        reg = self.build("codex", "Dispatch implementation to Codex via codex-rescue.")
        self.assertEqual(doctor.check_routing(reg, self.repo), [])

    def test_disagreement_fails(self):
        reg = self.build("codex", "Implementation stays in-session on Claude.")
        f = doctor.check_routing(reg, self.repo)
        self.assertEqual([x.code for x in f], ["routing-drift"])
        self.assertEqual([x.level for x in f], ["fail"])

    def test_claude_lane_agreement_passes(self):
        reg = self.build("claude", "Implementation stays in-session on Claude.")
        self.assertEqual(doctor.check_routing(reg, self.repo), [])
